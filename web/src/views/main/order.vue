<template>
  ...
  <div class="order-tickets">
    ...
    <a-row class="order-tickets-row"
           v-for="ticket in tickets"
           :key="ticket.passengerId">
      <a-col :span="2">{{ticket.passengerName}}</a-col>
      <a-col :span="6">{{ticket.passengerIdCard}}</a-col>
      <a-col :span="4">
        <a-select v-model:value="ticket.passengerType"
                  style="width: 100%">
          <a-select-option v-for="item in PASSENGER_TYPE_ARRAY"
                           :key="item.code"
                           :value="item.code">
            {{item.desc}}
          </a-select-option>
      ...
      <a-col :span="4">
        <a-select v-model:value="ticket.seatTypeCode"
                  style="width: 100%">
          <a-select-option v-for="item in seatTypes"
                           :key="item.code"
                           :value="item.code">
            {{item.desc}}
          </a-select-option>
  ...
  <div v-if="tickets.length > 0">
    <a-button type="primary"
              size="large"
              @click="finishCheckPassenger">提交订单</a-button>
  </div>
  ...
  <a-modal v-model:visible="visible"
    ...
    <div class="order-tickets">
      ...
      <div style="color: red">
        体验排队购票，加入多人一起排队购票：
        <a-input-number v-model:value="lineNumber"
                        :min="0"
                        :max="20" />
      </div>
      <!--<br/>-->
  ...
  <!-- 第二层验证码 后端 -->
  <a-modal v-model:visible="imageCodeModalVisible"
           :title="null"
           :footer="null"
           :closable="false"
    ...
    <p>
      <a-input v-model:value="imageCode"
               placeholder="图片验证码">
        <template #suffix>
    ...
    <a-button type="danger"
              block
              @click="handleOk">输入验证码后开始购票</a-button>
  </a-modal>

  <!-- 第一层验证码 纯前端 -->
  <a-modal v-model:visible="firstImageCodeModalVisible"
           :title="null"
           :footer="null"
           :closable="false"
           style="top: 50px; width: 400px">
    ...
    <p>
      <a-input v-model:value="firstImageCodeTarget"
               placeholder="验证码">
        <template #suffix>
    ...
    <a-button type="danger"
              block
              @click="validFirstImageCode">提交验证码</a-button>
  </a-modal>
  ...
  <a-modal v-model:visible="lineModalVisible"
    ...
    <br />
    <a-button type="danger"
              @click="onCancelOrder">取消购票</a-button>
  </a-modal>
</template>
